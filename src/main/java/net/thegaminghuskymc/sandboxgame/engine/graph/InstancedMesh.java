...
public class InstancedMesh extends Mesh {
    ...
    public void renderListInstancedBlocks(List<GameBlock> gameItems, Transformation transformation, Matrix4f viewMatrix) {
        renderChunkInstancedBlocks(gameItems, false, transformation, viewMatrix);
    }

    public void renderListInstancedBlocks(List<GameBlock> gameItems, boolean billBoard, Transformation transformation, Matrix4f viewMatrix) {
        initRender();
        ...
        for (int i = 0; i < length; i += chunkSize) {
            int end = Math.min(length, i + chunkSize);
            List<GameBlock> subList = gameItems.subList(i, end);
            renderChunkInstancedBlocks(subList, billBoard, transformation, viewMatrix);
        }
    ...
    private void renderChunkInstancedBlocks(List<GameBlock> gameItems, boolean billBoard, Transformation transformation, Matrix4f viewMatrix) {
        this.instanceDataBuffer.clear();

        int i = 0;

        Texture text = getMaterial().getTexture();
        for (GameBlock gameItem : gameItems) {
            Matrix4f modelMatrix = transformation.buildModelMatrix(gameItem);
            if (viewMatrix != null && billBoard) {
                viewMatrix.transpose3x3(modelMatrix);
            }
