...
import net.thegaminghuskymc.sandboxgame.engine.graph.anim.AnimGameBlock;
...
public class Renderer {
    ...
    private final List<GameBlock> filteredBlocks;
    ...
    private void setupParticlesShader() throws Exception {
        ...
        particlesShaderProgram.createVertexShader(Utils.loadResource("/assets/blockitect/shaders/particles_vertex.vs"));
        particlesShaderProgram.createFragmentShader(Utils.loadResource("/assets/blockitect/shaders/particles_fragment.fs"));
    ...
    private void setupSkyBoxShader() throws Exception {
        ...
        skyBoxShaderProgram.createVertexShader(Utils.loadResource("/assets/blockitect/shaders/sb_vertex.vs"));
        skyBoxShaderProgram.createFragmentShader(Utils.loadResource("/assets/blockitect/shaders/sb_fragment.fs"));
    ...
    private void setupSceneShader() throws Exception {
        ...
        sceneShaderProgram.createVertexShader(Utils.loadResource("/assets/blockitect/shaders/scene_vertex.vs"));
        sceneShaderProgram.createFragmentShader(Utils.loadResource("/assets/blockitect/shaders/scene_fragment.fs"));
    ...
    private void renderNonInstancedMeshes(Scene scene) {
        ...
        Map<Mesh, List<GameBlock>> mapMeshesBlocks = scene.getGameMeshesBlocks();
        for (Mesh mesh : mapMeshesBlocks.keySet()) {
            sceneShaderProgram.setUniform("material", mesh.getMaterial());
            ...
            mesh.renderListBlocks(mapMeshesBlocks.get(mesh), (GameBlock gameItem) -> {
                        sceneShaderProgram.setUniform("selectedNonInstanced", gameItem.isSelected() ? 1.0f : 0.0f);
                        Matrix4f modelMatrix = transformation.buildModelMatrix(gameItem);
                        sceneShaderProgram.setUniform("modelNonInstancedMatrix", modelMatrix);
                        if (gameItem instanceof AnimGameBlock) {
                            AnimGameBlock animGameItem = (AnimGameBlock) gameItem;
                            AnimatedFrame frame = animGameItem.getCurrentFrame();
                            sceneShaderProgram.setUniform("jointsMatrix", frame.getJointMatrices());
                        }
                    }
    ...
    private void renderInstancedMeshes(Scene scene, Matrix4f viewMatrix) {
        ...
        // Render each mesh with the associated game Items
        Map<InstancedMesh, List<GameItem>> mapMeshesItems = scene.getGameInstancedMeshes();
        for (InstancedMesh mesh : mapMeshesItems.keySet()) {
            Texture text = mesh.getMaterial().getTexture();
            if (text != null) {
                sceneShaderProgram.setUniform("numCols", text.getNumCols());
                sceneShaderProgram.setUniform("numRows", text.getNumRows());
            ...
            filteredItems.clear();
            for (GameItem gameItem : mapMeshesItems.get(mesh)) {
        ...
        // Render each mesh with the associated game Items
        Map<InstancedMesh, List<GameBlock>> mapMeshesBlocks = scene.getGameInstancedMeshesBlocks();
        for (InstancedMesh mesh : mapMeshesBlocks.keySet()) {
            Texture text = mesh.getMaterial().getTexture();
            if (text != null) {
                sceneShaderProgram.setUniform("numCols", text.getNumCols());
                sceneShaderProgram.setUniform("numRows", text.getNumRows());
            ...
            for (GameBlock gameItem : mapMeshesBlocks.get(mesh)) {
                if (gameItem.isInsideFrustum()) {
                    filteredBlocks.add(gameItem);
                }
            }

            shadowRenderer.bindTextures(GL_TEXTURE2);

            mesh.renderListInstancedBlocks(filteredBlocks, transformation, viewMatrix);
        }
    }
